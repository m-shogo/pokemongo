...
- 右下パネルの「Beta」バッジ横にある `?` ボタンを押すと、初心者向けの使い方ガイドがポップアップで表示されます。
...
4. 9db の IV 計算ページを再読み込みすると、右下に「IV OCR」パネルが表示されます。
  - 9db ページ右下パネルの「?」→「コードをコピー」ボタンを押すと、最新スクリプト全文がクリップボードへ入るのでコピペが簡単です。
...
5. 「自動入力」を ON にすると、安定して読み取れた数値が自動的に入力欄へ転記されます。
  ...
  - 「?」ヘルプボタンを押すと、操作手順の要約（キャプチャ→校正→保存→自動入力）が見やすいリストで開きます。操作中に迷ったら参考にしてください。
