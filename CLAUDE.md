...
### 環境制約
- この環境では npm registry・外部API・GitHub raw 等への HTTP アクセスが全て 403 (host_not_allowed) でブロックされる
- npm install / npm run build はローカルで実行不可。ビルドは GitHub Actions (deploy.yml) に任せる
- 外部データ取得が必要な場合は、手書きで対応するか、ユーザーがローカルで取得してコミットする

### トークン節約
- 並行エージェント(Task)を大量に立てない。1ファイルずつ順番に書く方が効率的
- 大量データ生成(ポケモン全種族値等)は1世代ずつ確認しながら進める
- 不要な探索・リサーチを繰り返さない。ダメだったらすぐ別アプローチに切り替える
- 会話は簡潔に。説明過多にしない

### コード品質 (レビュー対策)
- 未使用コード(CSS・コンポーネント・import)は必ず削除
- CSS変数は定義済みのものを使う（--border ではなく --surface-border 等）
- コミットは機能単位で分割し、日本語で「何を・なぜ」を書く
- 変更したら git status → git add (具体ファイル指定) → commit → push を必ず行う

### データ管理方針
- ポケモンデータは世代別ファイル (pokemon/gen1.ts〜gen9.ts) + index.ts で管理
- 新世代追加時: genX.ts 作成 → index.ts に import 追加のみ
- evolution.ts に未登録のポケモンは単体計算フォールバックで動作する
